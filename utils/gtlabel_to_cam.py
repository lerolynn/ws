...
for i, id in enumerate(tqdm(ids)):
    if i == 0:
        label = np.array(Image.open('../data/VOC2012/SegmentationClass_gt/%s.png' % id))
        keys = np.unique(label)
        keys = np.delete(keys, np.where(keys == 255))

        high_res_gt = np.zeros((keys.shape[0], label.shape[0], label.shape[1]))
        mask = np.array([label == key for key in keys])
        print(mask.shape)
        high_res_gt[mask] = 1
        print(high_res_gt)

        high_res_shape = high_res_gt.shape
